// ROS2 PORT: mutex for obs_history_tensor is declared in State_RL_test.h
/**********************************************************************
                ...
                :FSMState(ctrlComp, FSMStateName::RL, "RL")
{
    ...
    gravity(2,0) = -0.98;
    //在构造函数中初始化，订阅
    this->Sub_=nh.subscribe<geometry_msgs::msg::Twist>("/cmd_vel", 1,  // FIX Bug9: was 1000boost::bind(&FSMState::cmdVelCallback,this,_1));
...
void State_RL::enter(){
    ...
    // FIX Bug11: real robot motor commands — uncomment ONLY after hardware validation
    // else if(real == true)
    // {
        for(int i=0; i<12; i++){
...
void State_RL::exit(){
    _percent = 0;
    if (debug == true) { ampthreadRunning = State_RL::STOP; amp_obs_thread->join(); }  // FIX: was unconditional
...
{
    // FIX Bug10: was hardcoded stair policy
    rclcpp::Node::SharedPtr _n = rclcpp::Node::make_shared("_tmp");
    _n->declare_parameter("policy_path", std::string("policy_flat.pt"));
    model_path = _n->get_parameter("policy_path").as_string();
