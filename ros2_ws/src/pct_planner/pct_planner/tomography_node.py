# ROS2 PORT: tomography.py → tomography_node.py
# Bugs fixed: Bug32 (benchmark loop), Bug33 (cfg NameError)
#!/usr/bin/python3
import os
import sys
import time
...
class Tomography(object):
    def __init__(self, cfg, scene_cfg):
        self.cfg = cfg  # FIX Bug33: was never stored, caused NameError in initROS
    ...
    def loadPCD(self, pcd_file):
        ...
        print("PCD points: %d", points.shape[0])
        ...
        print("Map center: [%.2f, %.2f]", self.center[0], self.center[1])
        print("Dim_x: %d", self.map_dim_x)
        print("Dim_y: %d", self.map_dim_y)
        print("Num slices init: %d", n_slice_init)
    ...
    def process(self, points):        
        ...
        print("Num slices simp: %d", layers_g.shape[0])
        print("Num repeats (for benchmarking only): %d", n_repeat)
        print(" -- avg t_map  (ms): %f", t_map / n_repeat)
        print(" -- avg t_trav (ms): %f", t_trav / n_repeat)
        print(" -- avg t_simp (ms): %f", t_simp / n_repeat)
        print(" -- avg t_all  (ms): %f", t_all / n_repeat)
    ...
    def exportTomogram(self, tomogram, map_file):        
        ...
        print("Tomogram exported: %s", file_name)
