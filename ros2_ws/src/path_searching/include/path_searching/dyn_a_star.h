...
// ROS2 PORT: replaced ros/ros.h + ros/console.h with rclcpp
#include <rclcpp/rclcpp.hpp>
#include <Eigen/Eigen>
#include <plan_env/grid_map.h>
#include <queue>
#include <limits>  // FIX Bug27: needed for numeric_limits

// FIX Bug27: was `1 >> 20` = 0, making A* behave like BFS
...
private:
    ...
    inline void coord2gridIndexFast(const double x, const double y, const double z,
    ...
    std::vector<GridNodePtr> path_nodes_;
    std::priority_queue<GridNodePtr, std::vector<GridNodePtr>, NodeComparator> open_set_;
    GridNodePtr ***GridNodeMap_;

    int rounds_{0};

    // ROS2 PORT: store node for logging
    rclcpp::Logger logger_ = rclcpp::get_logger("dyn_a_star");
...
inline bool AStar::Coord2Index(const Eigen::Vector3d &pt, Eigen::Vector3i &idx) const
{
    idx = ((pt - center_) * inv_step_size_ +
           Eigen::Vector3d(0.5, 0.5, 0.5)).cast<int>() + CENTER_IDX_;

    if (idx(0) < 0 || idx(0) >= POOL_SIZE_(0) ||
        idx(1) < 0 || idx(1) >= POOL_SIZE_(1) ||
        idx(2) < 0 || idx(2) >= POOL_SIZE_(2))
    {
        // ROS2 PORT: ROS_ERROR → RCLCPP_ERROR
        RCLCPP_ERROR(logger_, "Ran out of pool, index=%d %d %d", idx(0), idx(1), idx(2));
        return false;
    }
    return true;
}
