...
namespace ego_planner
{
  ...
  pp_.max_vel_ = node->get_parameter("manager/max_vel").get_value<decltype(pp_.max_vel_)>();
    node->declare_parameter("manager/max_acc", -1.0);
  pp_.max_acc_ = node->get_parameter("manager/max_acc").get_value<decltype(pp_.max_acc_)>();
    node->declare_parameter("manager/max_jerk", -1.0);
  pp_.max_jerk_ = node->get_parameter("manager/max_jerk").get_value<decltype(pp_.max_jerk_)>();
    node->declare_parameter("manager/feasibility_tolerance", 0.0);
  pp_.feasibility_tolerance_ = node->get_parameter("manager/feasibility_tolerance").get_value<decltype(pp_.feasibility_tolerance_)>();
    node->declare_parameter("manager/control_points_distance", -1.0);
  pp_.ctrl_pt_dist = node->get_parameter("manager/control_points_distance").get_value<decltype(pp_.ctrl_pt_dist)>();
    node->declare_parameter("manager/planning_horizon", 5.0);
  pp_.planning_horizen_ = node->get_parameter("manager/planning_horizon").get_value<decltype(pp_.planning_horizen_)>();
                                        ...
                                        Eigen::Vector3d local_target_vel, bool flag_polyInit, bool flag_randomPolyTraj)
  {
    ...
    ros::Time t_start = rclcpp::Clock{}.now();
    ...
    do
    {
      ...
      //重复inital
        ...
        double t;
        double t_cur = (rclcpp::Clock{}.now() - local_data_.start_time_).toSec();
        ...
        if (poly_time > ts)
        {
          ...
          for (t = ts; t < poly_time; t += ts)
          {
            ...
            {
              RCLCPP_ERROR(logger_, "pseudo_arc_length is empty, return!");
        ...
        do
        {
          cps_dist /= 1.5;
          point_set.clear();
          point_set.push_back(start_pt);//強制將第一個點為本身位置，軌跡就不會向前推演了
          sample_length = 0;
          id = 0;
          ...
          if (iteration_count>max_iterations){
            RCLCPP_ERROR(logger_, "max_iterations failure should never have happened");
            return false;
          }
    ...
    t_init = rclcpp::Clock{}.now() - t_start;
    ...
    if (!flag_step_1_success)
    {
      ...
      RCLCPP_ERROR(logger_, "Planning failure have happened %d times", Rebound_success_flag);
      return false;
    }
