// ROS2 PORT: EGOReplanFSM is now an rclcpp::Node subclass
EGOReplanFSM::EGOReplanFSM(const rclcpp::NodeOptions& options)
: rclcpp::Node("ego_planner_node", options)
{
  // ROS2 PORT: tf2 initialization
  ...
  void EGOReplanFSM::init(rclcpp::Node::SharedPtr node)
  {
    ...
    /*  fsm param  */
    node->declare_parameter("fsm/flight_type", -1);
  target_type_ = node->get_parameter("fsm/flight_type").get_value<decltype(target_type_)>();
    node->declare_parameter("fsm/thresh_replan", -1.0);
  replan_thresh_ = node->get_parameter("fsm/thresh_replan").get_value<decltype(replan_thresh_)>();
    node->declare_parameter("fsm/thresh_no_replan", -1.0);
  no_replan_thresh_ = node->get_parameter("fsm/thresh_no_replan").get_value<decltype(no_replan_thresh_)>();
    node->declare_parameter("fsm/planning_horizon", -1.0);
  planning_horizen_ = node->get_parameter("fsm/planning_horizon").get_value<decltype(planning_horizen_)>();
    node->declare_parameter("fsm/planning_horizen_time", -1.0);
  planning_horizen_time_ = node->get_parameter("fsm/planning_horizen_time").get_value<decltype(planning_horizen_time_)>();
    node->declare_parameter("fsm/emergency_time_", 1.0);
  emergency_time_ = node->get_parameter("fsm/emergency_time_").get_value<decltype(emergency_time_)>();

    node->declare_parameter("fsm/waypoint_num", -1);
  waypoint_num_ = node->get_parameter("fsm/waypoint_num").get_value<decltype(waypoint_num_)>();
    node->declare_parameter("fsm/odometry_topic", "/Odometry");
  odom_topic = node->get_parameter("fsm/odometry_topic").get_value<decltype(odom_topic)>();
    ...
    /* callback */
    exec_timer_ = nh.createTimer(rclcpp::Duration::from_seconds(0.2), &EGOReplanFSM::execFSMCallback, this);
    safety_timer_ = nh.createTimer(rclcpp::Duration::from_seconds(0.1), &EGOReplanFSM::checkCollisionCallback, this);

    odom_sub_ = nh.subscribe(odom_topic, 1, &EGOReplanFSM::odometryCallback, this);

    bspline_pub_ = node->create_publisher<quadrotor_msgs_ros2::msg::Bspline>("/planning/bspline", 10);
    data_disp_pub_ = node->create_publisher<quadrotor_msgs_ros2::msg::DataDisp>("/planning/data_display", 100);
    ...
    case TARGET_TYPE::MANUAL_TARGET:
      waypoint_sub_ = node->create_subscription<auto>("/waypoint_generator/waypoints", 1,
    [this](const auto msg){ waypointCallback(msg); });
      break;
    ...
    case TARGET_TYPE::REFENCE_PATH:
      //读取path话题生成全局轨迹
      path_sub_= node->create_subscription<auto>("/pct_path", 1,
    [this](const auto msg){ pathCallback(msg); });
      break;
    default:
  ...
  void EGOReplanFSM::planGlobalTrajbyGivenWps()
  {
    ...
    for (size_t i = 0; i < (size_t)waypoint_num_; i++)
    {
      ...
      rclcpp::Duration::from_seconds(0.001).sleep();
    }
  ...
  void EGOReplanFSM::pathCallback(const nav_msgs::msg::PathConstPtr &msg)
  {
    if (msg->poses.empty())
    {
      RCLCPP_WARN(get_logger(), "[EGO] Received empty path from move_base.");
      return;
    }

    if (!have_odom_)
    {
      RCLCPP_WARN(get_logger(), "[EGO] No odometry yet, cannot plan global trajectory.");
      return;
    }
    // === 1. 提取并转换 waypoints ===
  ...
  void EGOReplanFSM::waypointCallback(const nav_msgs::msg::PathConstPtr &msg)
  {
  ...
  void EGOReplanFSM::odometryCallback(const nav_msgs::msg::OdometryConstPtr &msg)
  {
    // odom_topic
   ...
   //将当前位姿从odom坐标系转换到map坐标，如果存在tf变换关系的话
    try
    {
      ...
      //将当前位姿从odom坐标系转换到map坐标，如果存在tf变换关系的话
      if (tf_listener_.waitForTransform("map", msg->header.frame_id, rclcpp::Time(0), rclcpp::Duration::from_seconds(3.0)));
      tf_listener_.lookupTransform("map", msg->header.frame_id,
                                  rclcpp::Time(0), transform_odom2map);
      // 位置变换    
  ...
  void EGOReplanFSM::execFSMCallback()
  {
    ...
    case EMERGENCY_STOP:
    {
      ...
      RCLCPP_ERROR(get_logger(), "emergency_stop_times :%d", emergency_stop_times);
...
;
  ...
  void EGOReplanFSM::checkCollisionCallback()
  {
    ...
    double t_cur = (rclcpp::Clock{}.now() - info->start_time_).toSec();
  ...
  bool EGOReplanFSM::callReboundReplan(bool flag_use_poly_init, bool flag_randomPolyTraj)
  {
    ...
    if (plan_success)
    {
      ...
      /* publish traj */
      quadrotor_msgs_ros2::msg::Bspline bspline;
      bspline.order = 3;
...
void EGOReplanFSM::getLocalTarget()
{
  ...
  RCLCPP_DEBUG(get_logger(), "start_pt_投影点时间t_proj=%.3f，投影点到start_pt_距离=%.3f m", 
