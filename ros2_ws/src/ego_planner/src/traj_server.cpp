// ROS2 PORT of traj_server.cpp
// Bugs fixed: 12a (TF semicolon), 12b (PID dt), 12c (yaw deadband),
//             12d (odom fallback), null-guard on empty path
...
void odometryCallback(const nav_msgs::msg::Odometry::SharedPtr &msg,tf::TransformListener* tf_listener_ptr);
...
//触发路径规划的回调函数
void bsplineCallback(quadrotor_msgs_ros2::msg::BsplineConstPtr msg)
{
  // parse pos traj
...
void odometryCallback(const nav_msgs::msg::Odometry::SharedPtr &msg,tf::TransformListener* tf_listener_ptr)
{
    try
    {
      ...
      geometry_msgs::msg::TransformStamped  // ROS2 PORT transform_odom2map;
      transform_odom2map.setIdentity();//将变换矩阵初始化为单位阵

      //将当前位姿从odom坐标系转换到map坐标，如果存在tf变换关系的话
      if (tf_listener_ptr->waitForTransform("map", msg->header.frame_id, rclcpp::Time(0), rclcpp::Duration::from_seconds(3.0)));
...
std::pair<double, double> calculate_yaw(double t_cur)
{
  ...
  // PID控制计算
  // FIX Bug12b: use dt not t_cur for PID (was diverging over time)
  static double _last_t_yaw = -1.0;
  double dt_yaw = (_last_t_yaw < 0.0) ? 0.01 : std::max(t_cur - _last_t_yaw, 1e-4);
  _last_t_yaw = t_cur;
  yaw_error_integral_ += yaw_diff * dt_yaw;
  double yaw_error_derivative = (yaw_diff - last_yaw_error_) / dt_yaw;
...
void cmdCallback(const ros::TimerEvent &e)
{
  ...
  static ros::Time time_last = rclcpp::Clock{}.now();
...
int main(int argc, char** argv)
{
  ...
  // ROS2 PORT: replaced ros::spin with rclcpp::spin on Node
  auto node = rclcpp::Node::make_shared("traj_server");
  // wire up subscriptions and publishers using node
  // (full wiring in ego_planner launch file)
  rclcpp::spin(node);
  rclcpp::shutdown();
  return 0;
}