...
// ROS2 PORT: All ROS1 includes replaced with ROS2 equivalents
...
// ROS2 PORT: message_filters uses rclcpp-based synchronizers
...
struct MappingParameters {
  ...
  // camera parameters (kept for depth image mode compatibility)
...
public:
  ...
  // ROS2 PORT: initMap takes rclcpp::Node::SharedPtr instead of ros::NodeHandle
...
private:
  ...
  // ROS2 PORT: tf2 replaces tf
  std::shared_ptr<tf2_ros::Buffer> tf_buffer_;
  std::shared_ptr<tf2_ros::TransformListener> tf_listener_;

  // ROS2 PORT: store node reference for timers, pubs, subs
  rclcpp::Node::SharedPtr node_;

  void cloudCallback(const sensor_msgs::msg::PointCloud2::SharedPtr msg);
  void odomCallback(const nav_msgs::msg::Odometry::SharedPtr odom);

  // ROS2 PORT: TimerEvent → rclcpp::TimerBase
  ...
  // ROS2 PORT: message_filters with rclcpp
  using SyncPolicyCloudOdom = message_filters::sync_policies::ApproximateTime<
      sensor_msgs::msg::PointCloud2, nav_msgs::msg::Odometry>;

  // ROS2 PORT: Subscribers and publishers use rclcpp API
  rclcpp::Subscription<sensor_msgs::msg::PointCloud2>::SharedPtr cloud_sub_;
  rclcpp::Subscription<nav_msgs::msg::Odometry>::SharedPtr odom_sub_;
  rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr map_pub_;
  rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr map_inf_pub_;

  // ROS2 PORT: ros::Timer → rclcpp::TimerBase
...
inline int GridMap::getOccupancy(Eigen::Vector3i id) {
  if (id(0)<0||id(0)>=mp_.map_voxel_num_(0)||id(1)<0||id(1)>=mp_.map_voxel_num_(1)||
      id(2)<0||id(2)>=mp_.map_voxel_num_(2)) return -1;
  return md_.occupancy_buffer_[toAddress(id)] > mp_.min_occupancy_log_ ? 1 : 0;
}
